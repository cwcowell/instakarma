...
class KarmaManager:
    ...
    def get_karma(self, conn: Connection, name: str) -> int:
        try:
            self.logger.debug(f"Asking DB for karma of name '{name}'")
            cursor: Cursor = self.db_manager.execute_query(conn,
                                                           """
SELECT karma
FROM entities
WHERE name = ? AND disabled = FALSE;""",
    ...
    def get_top_granters(self, conn: Connection, recipient_name: str) -> list[tuple[str, int]]:
        try:
...
LIMIT {NUM_TOP_GRANTERS};""",
    ...
    def get_top_recipients(self, conn: Connection, granter_name: str) -> list[tuple[str, int]]:
        try:
...
LIMIT {NUM_TOP_RECIPIENTS};""",
    ...
    def grant_karma(self, conn: Connection, granter_name: str, recipient_name: str, amount: int) -> None:
        if self.entity_manager.get_status_from_db(conn, recipient_name) == Status.DISABLED:
            self.logger.info(
                f"Can't grant '{amount}' karma from name '{granter_name}' to disabled name '{recipient_name}'")
            raise DisabledEntityError

        self.logger.debug(
            f"Adding grant of '{amount}' karma to DB from name '{granter_name}' to name '{recipient_name}'")
        try:
